...
async function main() {
    ...
    const stakingToken = YAM_YCRV_LP_TOKEN_ADDR;
    const stakingTokenTicker = "UNIV2";
    const rewardPoolAddr = YAM_YCRV_LP_YAM_CLASSIC_REWARD_POOL_ADDR;
    ...
    const totalYCRVInUniswapPair = await Y_TOKEN.balanceOf(YAM_YCRV_LP_TOKEN_ADDR) / 1e18;
    const totalYAMInUniswapPair = await YAM_TOKEN.balanceOf(YAM_YCRV_LP_TOKEN_ADDR) / 1e18;
    ...
    const stakingTokenPrice = (totalYAMInUniswapPair * rewardTokenPrice + totalYCRVInUniswapPair * YVirtualPrice) / totalSupplyOfStakingToken;
